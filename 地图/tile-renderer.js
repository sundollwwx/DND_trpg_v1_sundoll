...
(function initSundollTileRenderer(global) {
  ...
  const MATERIALS = {
    floor: { files: ['stone-floor.png', 'stone-floor-v2.png'], fallback: '#8f8574', crop: .34, overlay: 'rgba(22,27,33,.08)' },
    wood: { files: ['wood-floor.png', 'wood-floor-v2.png'], fallback: '#a97c50', crop: .34, overlay: 'rgba(46,26,10,.06)' },
    grass: { files: ['grass.png', 'grass-v2.png'], fallback: '#7da05c', crop: .34, overlay: 'rgba(20,45,18,.06)' },
    road: { files: ['dirt-road.png', 'dirt-road-v2.png'], fallback: '#a8754d', crop: .34, overlay: 'rgba(48,28,12,.04)' },
    water: { files: ['water.png', 'water-v2.png'], fallback: '#4f8fc2', crop: .34, overlay: 'rgba(7,32,53,.10)' },
    wall: { files: ['stone-wall.png', 'stone-wall-v2.png'], fallback: '#5c6068', crop: .34, overlay: 'rgba(8,12,15,.16)', wall: true },
  };
  ...
  function variantIndex(material, x, y) {
    return stableNumber(x, y, 79) % material.files.length;
  }

  function imageFor(id, x, y) {
    const material = MATERIALS[id];
    if (!material) return null;
    const file = material.files[variantIndex(material, x, y)];
    if (!images.has(file)) {
