...
window.CampaignJournal = (() => {
  ...
  let returnToBackpack=null,backButton;
  function open(options={}) {
    ...
    if (!dialog) {
      ...
      backButton=document.createElement('button');backButton.type='button';backButton.textContent='← 返回背包';
      backButton.addEventListener('click',()=>{requestCloseBook();if(!dialog.open)returnToBackpack?.();});
      dialog.querySelector('.journal-heading-actions').prepend(backButton);
    ...
    returnToBackpack=typeof options.onBack==='function'?options.onBack:null;backButton.hidden=!returnToBackpack;
    dialog.showModal();
