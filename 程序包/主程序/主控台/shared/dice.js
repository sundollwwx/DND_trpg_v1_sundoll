...
`;
  ...
  function showFallback(total, label, opts, rollId) {
    ...
    const board = opts.surface?.open ? opts.surface : document.getElementById('board');
    ...
    if(opts.surface?.open){root.style.position='absolute';root.style.left='0';root.style.top='0';opts.surface.appendChild(root);}else document.body.appendChild(root);
    rootEl = root;
  ...
  function runDieAnimation(sides, label, total, opts, rollId) {
    ...
    const bonusDice=Array.isArray(opts.extraDice)?opts.extraDice.slice(0,10):[];
    for(const bonus of bonusDice){
      if(!bonus||![4,6,8,10,12,20].includes(bonus.sides)||!Number.isInteger(bonus.value)||bonus.value<1||bonus.value>bonus.sides)continue;
      const bonusPoly=POLY[dieKey(bonus.sides)];display.push({faceIdx:resultLabel(bonusPoly,bonus.value),text:String(bonus.value),rollIndex:-1,bonus:true,sides:bonus.sides});
    }
    ...
    const requestedSizeScale = Number(opts.sizeScale);
    const sizeScale = Number.isFinite(requestedSizeScale) ? Math.max(.5, Math.min(3, requestedSizeScale)) : 1;
    const perScale = countScale * screenScale * DICE_SIZE_MULTIPLIER * sizeScale;
    // 大骰子的中心高于桌面，抬高取景中心以免结果被弹窗上沿裁切。
    if (opts.surface?.open && sizeScale > 1) camera.lookAt(0, perScale * .8, 0);
    ...
    function supportHeight(quaternion, scaleValue, shape=poly) {
      let minimumY = Infinity;
      shape.verts.forEach((vertex) => {
    ...
    function bodyMaterial(faceLength, faceLabel, highlighted, dimmed, materialKey=key) {
      const cacheKey = `${skin.key}|${materialKey}|regular|${faceLength}|${faceLabel}|${highlighted ? 1 : 0}|${dimmed ? 1 : 0}`;
      if (bodyMaterialCache.has(cacheKey)) return bodyMaterialCache.get(cacheKey);
      const material = finishBodyMaterial(faceTexture(highlighted, faceLength, faceLabel, skin, materialKey), dimmed);
    ...
    const diceData = display.map((d, i) => {
      const dieSides=d.sides||sides,key=dieKey(dieSides),poly=POLY[key]||POLY.d20,isD4=key==='d4';
      let dieGeo=geo;if(d.bonus){dieGeo=isD4?buildD4Geometry(poly):buildGeometry(poly);dieGeo.computeBoundingSphere();disposeQueue.push(dieGeo);}
      const res = d.faceIdx;
      const selected = !!d.bonus || !mode || d.rollIndex === pick;
      const discarded = !d.bonus && !!mode && !selected;
      // 每颗骰子每个面的文字：普通骰 1..N；d100 十位骰 00/10/…90、个位骰 0-9
      const faceTexts = dieSides === 100
        ? (i % 2 === 0
      ...
      // 不能把 27 错画成 7。
      if (dieSides !== 100 && faceTexts[res - 1] !== String(d.text)) faceTexts[res - 1] = String(d.text);
      ...
      const mats = isD4
        ...
        : poly.faces.map((face, faceIndex) => bodyMaterial(face.length, faceTexts[faceIndex], faceIndex === res - 1, false,key));
      ...
      const initialFloorY = supportHeight(mesh.quaternion, perScale,poly);
      ...
      const resultScale = perScale * (mode&&!d.bonus ? (selected ? 1.18 : .76) : 1);
      const finalSupportY = supportHeight(qFinal, finalScale,poly);
      return {
        poly,isD4,dieSides,bonus:!!d.bonus,
        mesh,
        shadowBlob,
        selectionRing,
        res,
    ...
    function prepareSettleTargets() {
      ...
      diceData.forEach((d, index) => {
        d.finalPosition.set(targets[index].x, supportHeight(d.qFinal, d.finalScale,d.poly), targets[index].z);
        if (d.selectionRing) {
    ...
    root.dataset.diceSides = diceData.map((d) => d.dieSides).join(',');
    ...
    function applyResultFocus(d, progress) {
      ...
      d.mesh.position.y = supportHeight(d.mesh.quaternion, scale,d.poly);
      if (d.selectionRing) {
    ...
    function frame(now) {
      ...
      diceData.forEach((d) => {
        if (inPhysics) {
          ...
          const floorY = supportHeight(d.mesh.quaternion, d.mesh.scale.x,d.poly);
          ...
          if (!d.settlePosition) {
            ...
            d.settleClearance = Math.max(0, d.settlePosition.y - supportHeight(d.settleQuaternion, d.settleScale,d.poly));
          ...
          const currentSupportY = supportHeight(d.mesh.quaternion, d.mesh.scale.x,d.poly);
      ...
      if (elapsed < animationDuration) {
        ...
        const topObservations = diceData.map((d) => {
          ...
          const candidates = d.isD4 ? d.poly.verts : d.poly.normals;
          ...
          return {
            label: d.isD4 ? String(bestIndex + 1) : String(d.faceTexts[bestIndex]),
            score: bestScore,
          };
        });
        ...
        const diagnostic = {
          rollId,
          diceSides: diceData.map(d=>d.dieSides),
          ...
          groundClearances: diceData.map((d) => Math.round((d.mesh.position.y - supportHeight(d.mesh.quaternion, d.mesh.scale.x,d.poly)) * 10000) / 10000),
        ...
        const detailText = opts.detail || (mode !== 0 && rawDice.length >= 2
